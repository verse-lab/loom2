...
abbrev HeapM := PredTrans hProp EPost⟨⟩
...
def HeapM.inhale (hp : hProp) : HeapM Unit := fun post _ => hp -∗ post ()
...
instance : WPMonad HeapM hProp EPost⟨⟩ where
  wpTrans x post _ := ∀ʰ H, H -∗ x (H ∗ post ·) epost⟨⟩
  wp_trans_pure x post _   := by sorry
  wp_trans_bind x f post _ := by sorry
  wp_trans_monotone x      := by sorry

theorem HeapM.frame (H pre : hProp) (post : α → hProp) (x : HeapM α) :
  ⦃ pre ⦄ x ⦃ post ⦄ →
  ⦃ H ∗ pre ⦄ x ⦃ v, H ∗ post v ⦄ := by sorry

theorem HeapM.inhale_spec (hp : hProp) :
  ⦃ ∅ ⦄ inhale hp ⦃ _, hp ⦄ := by sorry
