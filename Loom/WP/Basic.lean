...
attribute [reducible, instance] AssertionLang.cl

instance (priority := high) : AssertionLang Prop where
  cl := inferInstance

instance (priority := high) {l : Type u} {σ : Type v} [AssertionLang l] : AssertionLang (σ → l) where
  cl := inferInstance

instance [AssertionLang l] : PartialOrder l := inferInstance

class ExcpetAssertionLang (e : Type w) where
  as : AssertionLang e

instance [ExcpetAssertionLang e] : CompleteLattice e := ExcpetAssertionLang.as.cl

instance : ExcpetAssertionLang Prop where
  as := inferInstance

instance [ExcpetAssertionLang e] : CCPO e where
  has_csup {c} _ := ExcpetAssertionLang.as.cl.has_sup c

instance : ExcpetAssertionLang EPost.nil where
  as := ⟨inferInstance⟩

instance (priority := high) [ExcpetAssertionLang eh] [ExcpetAssertionLang et] : ExcpetAssertionLang (EPost.cons eh et) where
  as := ⟨inferInstance⟩

instance (priority := high) {ε : Type u} {l : Type v} [AssertionLang l] : ExcpetAssertionLang (ε → l) where
  as := ⟨inferInstance⟩
...
class WPMonad (m : Type u → Type v) (l : outParam (Type w)) (el : outParam (Type w'))
    [Monad m] extends LawfulMonad m, AssertionLang l, ExcpetAssertionLang el where
...
theorem WPMonad.wp_bind [Monad m] [WPMonad m l el] (x : m α) (f : α → m β) (post : β → l) (epost : el) :
    wp x (fun x => wp (f x) post epost) epost ⊑ wp (x >>= f) post epost := by apply wp_trans_bind x f

theorem WPMonad.wp_cons [Monad m] [WPMonad m l el] (x : m α) (post post' : α → l) (epost : el)
   (h : post ⊑ post') :
    wp x post epost ⊑ wp x post' epost := by solve_by_elim [WPMonad.wp_trans_monotone, PartialOrder.rel_refl]

theorem WPMonad.wp_cons_econs [Monad m] [WPMonad m l el] (x : m α) (post post' : α → l) (epost epost' : el)
   (h : post ⊑ post') (h' : epost ⊑ epost') :
    wp x post epost ⊑ wp x post' epost' := by
  exact WPMonad.wp_trans_monotone x post post' epost epost' h' h

theorem WPMonad.wp_econs [Monad m] [WPMonad m l el] (x : m α) (post : α → l) (epost epost' : el)
  (h' : epost ⊑ epost') :
    wp x post epost ⊑ wp x post epost' := by solve_by_elim [WPMonad.wp_trans_monotone, PartialOrder.rel_refl]

theorem WPMonad.wp_econs_bot [Monad m] [WPMonad m l el] (x : m α) (post : α → l) (epost : el) :
    wp x post ⊥ ⊑ wp x post epost := by solve_by_elim [WPMonad.wp_econs, bot_le]

theorem WPMonad.wp_cons_rel [Monad m] [WPMonad m l el] (x : m α) (post post' : α → l) (epost : el)
    (h : post ⊑ post') {pre : l} (h' : pre ⊑ wp x post epost) :
    pre ⊑ wp x post' epost :=
  PartialOrder.rel_trans h' (WPMonad.wp_cons x post post' epost h)

theorem WPMonad.wp_econs_rel [Monad m] [WPMonad m l el] (x : m α) (post : α → l) (epost epost' : el)
    (h : epost ⊑ epost') {pre : l} (h' : pre ⊑ wp x post epost) :
    pre ⊑ wp x post epost' :=
  PartialOrder.rel_trans h' (WPMonad.wp_econs x post epost epost' h)

theorem WPMonad.wp_econs_bot_rel [Monad m] [WPMonad m l el] (x : m α) (post : α → l) (epost : el)
...
@[simp, grind =]
theorem apply_pushExcept {α ε l el} (x : PredTrans l el (Except ε α)) (post : α → l) (epost : EPost.cons (ε → l) el) :
  (PredTrans.pushExcept x) post epost = x (epost.pushExcept post) epost.tail := rfl

/- TODO: change the order of `(el × (ε → l))` -/
...
@[simp, grind =]
theorem ExceptT.apply_wp {α ε l el} [Monad m] [WPMonad m l el] (x : ExceptT ε m α) (post : α → l) (epost : EPost.cons (ε → l) el) :
...
@[simp, grind =]
theorem StateT.apply_wp {σ : Type u} [Monad m] [WPMonad m l el] (x : StateT σ m α) (post : α → σ → l) (epost : el) (s : σ) :
...
@[simp, grind =]
theorem ReaderT.apply_wp {ρ : Type u} [Monad m] [WPMonad m l el] (x : ReaderT ρ m α) (post : α → ρ → l) (epost : el) (r : ρ) :
  (wp x) post epost r = wp (x.run r) (fun a => post a r) epost := rfl

/-
TODO: Same as for pushExcept
instance OptionT.instWPMonad {l : Type u}
  [CompleteLattice l] [Monad m] [WPMonad m l el] :
  WPMonad (OptionT m) l (el × l) where
  wpImpl x post epost := WPMonad.wp (m := m) (l := l) (el := el) x.run
