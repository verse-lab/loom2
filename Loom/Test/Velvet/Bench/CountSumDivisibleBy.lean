...
method countSumDivisibleBy (n : Nat) (d : Nat)
  return (result : Nat)
  require d > 0
  ensures True  -- result = (Finset.filter (fun k => d ∣ digitSum k) (Finset.range n)).card  (needs Mathlib)
  do
  let mut count := 0
  let mut k := 0
  while' k < n
    ...
    invariant True  -- count = (Finset.filter (fun i => d ∣ digitSum i) (Finset.range k)).card
    done_with k = n
  do
    let sum := digitSum k
    if sum % d = 0 then
