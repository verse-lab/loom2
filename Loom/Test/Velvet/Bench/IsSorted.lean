...
method isSorted (a : Array Int) return (result : Bool)
  ...
  do
    ...
    while' i + 1 < a.size ∧ sorted
      invariant i ≤ a.size
      invariant (sorted = true → ∀ k : Nat, k < i → a[k]! ≤ a[k + 1]!)
      invariant (sorted = false → ∃ k : Nat, k ≤ i ∧ k + 1 < a.size ∧ a[k]! > a[k + 1]!)
...
@[grind]
theorem adjacent_implies_global (a : Array Int) (hadj : AdjacentSorted a) : GloballySorted a := by
  intro p q hpq
