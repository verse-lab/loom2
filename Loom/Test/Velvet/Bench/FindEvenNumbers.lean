...
method findEvenNumbers (arr : Array Int)
  ...
  while' i < arr.size
    ...
    invariant (∀ x, x ∈ result → isEvenInt x = true)
    invariant (∀ x, isEvenInt x = false → result.count x = 0)
    invariant (∀ x, isEvenInt x = true → result.count x = (arr.take i).count x)
    invariant (indices.size = result.size)
    invariant (∀ k, k < indices.size → indices[k]! < i)
    invariant (∀ k, k < indices.size → indices[k]! < arr.size)
    invariant (∀ k, k < indices.size → result[k]! = arr[indices[k]!]!)
    invariant (∀ k j, k < j → j < indices.size → indices[k]! < indices[j]!)
