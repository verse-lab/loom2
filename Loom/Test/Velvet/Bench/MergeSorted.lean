...
@[grind]
def MergeSorted.isSorted (arr : Array Nat) : Prop :=
  ∀ (i j : Nat), i < j → j < arr.size → arr[i]! ≤ arr[j]!

theorem isSorted_le (arr : Array Nat) (i : Nat) :
  MergeSorted.isSorted arr → i + 1 < arr.size → arr[i]! ≤ arr[i + 1]! := by
...
method mergeSorted (a1 : Array Nat) (a2 : Array Nat)
  ...
  require MergeSorted.isSorted a1
  require MergeSorted.isSorted a2
  ensures result.size = a1.size + a2.size
  ensures MergeSorted.isSorted result
  ...
  while' i < a1.size ∨ j < a2.size
    ...
    invariant MergeSorted.isSorted result
    ...
    invariant ∀ v : Nat, result.count v = (a1.take i).count v + (a2.take j).count v
    ...
    invariant result.size > 0 → i < a1.size → result[result.size - 1]! ≤ a1[i]!
    -- Invariant 6: Last element of result is ≤ remaining elements in a2
    invariant result.size > 0 → j < a2.size → result[result.size - 1]! ≤ a2[j]!
...
theorem count_take [DecidableEq α] [Inhabited α] {a : α} {xs : Array α} :
  ...
  intro; grind [Array.extract_succ_right, getElem!_neg, getElem!_pos]

-- @[grind]
-- theorem Array.take_size {xs : Array α}  : i >= xs.size -> xs.take i = xs := by
--   cases xs
--   simp
