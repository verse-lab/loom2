...
meta def mkBackwardRuleForSplit
    (splitInfo : SplitInfo) (wpHead m Pred errTy monadInst instAL instEAL instWPMonad : Expr)
  ...
  let prf ←
    ...
    let mkWP (prog : Expr) : Expr :=
      mkAppN (mkAppN wpHead #[m, Pred, errTy, monadInst, instAL, instEAL, instWPMonad, a, prog, post, epost]) ss
...
private meta def testSplitBackwardRule
    (m Pred errTy monadInst instAL instEAL instWPMonad : Expr) (prog : Expr) (excessArgs : Array Expr)
    : MetaM Expr := do
  ...
  let rule ← SymM.run do
    mkBackwardRuleForSplit splitInfo wpHead m Pred errTy monadInst instAL instEAL instWPMonad excessArgs
  inferType rule.expr
...
private meta def withStateMNat (k : Expr → Expr → Expr → Expr → Expr → Expr → Expr → MetaM α) : MetaM α := do
  ...
  k m Pred errTy monadM instAL instEAL instWPMonad

-- Test 1: ite split for StateM Nat
-- `ite c (alt₁ : StateM Nat Unit) (alt₂ : StateM Nat Unit)` with 1 excess arg
#eval! show MetaM Unit from withStateMNat fun m Pred errTy monadInst instAL instEAL instWPMonad => do
  let nat := mkConst ``Nat
  ...
  withLocalDeclD `s nat fun s => do
    ...
    let ty ← testSplitBackwardRule m Pred errTy monadInst instAL instEAL instWPMonad prog #[s]
