...
def HeapM.bind (x : HeapM α) (f : α → HeapM β) : HeapM β :=
  { predTrans := ⟨fun post epost => x.predTrans.apply (fun a => (f a).predTrans.apply post epost) epost⟩ }
...
instance : WPMonad HeapM hProp EPost⟨⟩ where
  wpTrans x := ⟨fun post _ => ∀ʰ H, H -∗ x.predTrans.apply (fun a => (H ∗ (post a))) epost⟨⟩⟩
...
theorem HeapM.frame (H pre : hProp) (post : α → hProp) (x : HeapM α) :
  ...
  ⦃ H ∗ pre ⦄ x ⦃ v, H ∗ post v ⦄ := by
    ...
    have step3 : (x.predTrans.apply (fun v => (H_1 ∗ H) ∗ post v) epost⟨⟩) heap :=
    hForall_star_elim (H_1 ∗ H) (P := fun H' => H' -∗ x.predTrans.apply (fun v => H' ∗ post v) epost⟨⟩) hWand_elim heap step2
    revert step3
    apply x.monotone
    rfl
    intro v
